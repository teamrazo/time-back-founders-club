"use client";
import React from "react";
import { cn } from "@/lib/utils";

interface FidaMessageProps {
  feeling?: string;
  identity?: string;
  dream?: string;
  action?: string;
  className?: string;
  variant?: 'default' | 'subtle';
}

export function FidaMessage({ feeling, identity, dream, action, className, variant = 'default' }: FidaMessageProps) {
  return (
    <div className={cn(
      "rounded-xl border p-4 space-y-2",
      variant === 'default'
        ? "border-brand-primary/15 bg-brand-primary/5"
        : "border-brand-border bg-brand-card",
      className
    )}>
      {feeling && (
        <p className="text-sm text-brand-fg/80 leading-relaxed">
          <span className="text-xs font-semibold text-brand-primary uppercase tracking-wide mr-1">Feel:</span>
          {feeling}
        </p>
      )}
      {identity && (
        <p className="text-sm text-brand-fg/80 leading-relaxed">
          <span className="text-xs font-semibold text-brand-primary400 uppercase tracking-wide mr-1">Identity:</span>
          {identity}
        </p>
      )}
      {dream && (
        <p className="text-sm text-brand-fg/80 leading-relaxed">
          <span className="text-xs font-semibold text-emerald-400 uppercase tracking-wide mr-1">Dream:</span>
          {dream}
        </p>
      )}
      {action && (
        <p className="text-sm text-brand-fg/80 leading-relaxed">
          <span className="text-xs font-semibold text-amber-400 uppercase tracking-wide mr-1">Action:</span>
          {action}
        </p>
      )}
    </div>
  );
}
